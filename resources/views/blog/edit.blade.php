<x-layout title="Update Blog">
    <div class="special_header">
        <span>
            <i class="fal fa-plus"></i>
            {{ __('layout.update_blog') }}
        </span>
    </div>
    <div class="form-data">
        <form action="/blog/{{ $blog->id }}" method="POST" class="create_form" enctype="multipart/form-data">
            @csrf
            @method("PATCH")
            <div class="profile-image-container form-group img-thumbnail flex-1">
                <div class="profile-image-circle" style="{{ $blog->thumbnail ? 'display:none;' : ''}}">
                ...
                <div id="imagePreview">
                    @if ($blog->thumbnail)
                        <img src="{{ asset("storage/". $blog->thumbnail) }}" alt="Profile Image" />
                    @endif
                </div>
                @error('thumbnail')
            ...
            <div class="form-group required">
                ...
                <input type="text" name="title_ar" id="title_ar" value="{{ old("title_ar", $blog->title_ar) }}">
                @error('title_ar')
                    <p class="error">{{ $message }}</p>
                @enderror
            </div>
            <div class="form-group optional">
                ...
                <input type="text" name="title_en" id="title_en" value="{{ old("title_en", $blog->title_en) }}">
                @error('title_en')
                    <p class="error">{{ $message }}</p>
                @enderror
            </div>
            <div class="form-group multi-select select optional">
                ...
                @if ($blog->tags->count() > 0)
                    <input type="hidden" name="tags" value="{{ collect($blog->tags)->map(fn($tag) => $tag->id)->implode(',') }}">
                @else
                ...
                <div class="box">
                    ...
                    <div class="options">
                        @foreach ($tags as $tag)
                            <div class="option {{ $blog->tags->contains($tag) ? 'selected' : '' }}" data-value="{{ $tag->id }}">
                                {{ $tag->name }}
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="form-group required flex-1">
                ...
                <textarea name="body_ar" id="content_ar" cols="30" rows="10">{{ old('body_ar', $blog->body_ar) }}</textarea>
            </div>
            <div class="form-group optional flex-1">
                <label for="content_en">
                    {{ __('layout.body_en') }}
                </label>
                <textarea name="body_en" id="content_en" cols="30" rows="10">{{ old('body_en', $blog->body_en) }}</textarea>
            </div>


            <div class="form-group optional flex-1">
                <label for="files">
                    {{ __('layout.upload_attachments') }}
                </label>
            ...
            <div class="form-group flex-1">
                <button type="submit">{{ __('layout.update_blog') }}</button>
            </div>
        </form>
